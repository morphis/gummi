...
workflow** — plan → implement → verify — with each work item on its own git
...
Re-run `gummi doctor` until it reports `ready: true`, then proceed. It also
exits **7** while any check is failing and 0 once the workspace is ready, in
both output shapes, so `gummi doctor --json > d.json; ec=$?` branches without
parsing. 7 sits outside the run/resume exit table below, so a readiness
failure can never be mistaken for a drive's typed status.
...
A run drives one feature through the workflow: plan → implement → verify.
There is one route — plan is the design stage (it explores the problem,
converges on an approach and writes the implementation plan), implement is the
coding stage, and verify runs the checks. Each stage ends with a critique pass
before its gate. The design gate auto-crosses under `--gate-approval=autopilot`;
the default, `attended`, stops for you to approve it.

`status`, `watch`, `spec`, and `diff` are **read-only** — they take no lock, so
you can inspect a feature while a run is live. `status` answers *where* a card
is (a store snapshot, updated once per turn); `gummi watch <id>` follows what it
is doing *right now* — the agent's own transcript, tool calls, and state
changes, streamed off the driving process's live file. `watch --json` emits that
...
redundant with the stream: the **last NDJSON line's `event`** (`verified`,
...
verb. `verified` carries no `next` (nothing to resume); `blocked` carries none
...
2. Read the exit code:
   - **verified (0)** — parse the final `verified` line for the branch; report it to your
   ...
   - **question (2)** — a design decision was delegated to you. Answer from your
     **task context first**; if unsure, read the spec (`gummi spec <id>`) or diff
     ...
     `gummi resume <id> --answer "<text-or-option>"`. A design gate (the
     default, `--gate-approval=attended`) is the same shape: `resume <id> --approve` or
...
3. After a `resume`, read the new exit code and repeat until `verified`.
...
wait (`gummi watch <id>` follows that run's live agent stream; `tail -f
.gummi/state/events.jsonl` shows milestones only) until the
...
- `--approve` / `--request-changes "<note>"` — decides a **`gate`** event
  (a design gate, which `--gate-approval=attended` hands back to you), and the same pair continues a
  `--until` **`stopped`** run.
...
tokens, add `--until plan`. The run stops cleanly at the design stage — event
`stopped`, exit 0 — with the card parked and resumable. Hand the spec to the
human (`gummi spec <id>`); once approved, `gummi resume <id> --approve`
continues to the verified branch. `plan` is the only stop `--until` accepts:
the design gate is the one boundary where stopping is unambiguous.

`--until` is per-invocation only — it is never persisted on the card, so a bare
`resume --approve` with no `--until` runs straight through to the end.

## Research cards: `gummi research`

`gummi research "<brief>"` mints one RS card from a free-form brief and drives
it headlessly to the decompose gate. It runs the same stages every card does,
with two differences: its implement stage is read-only (it gathers evidence
rather than writing code) and it never gets a worktree. `--until plan` is the
only pre-decompose stop. At the gate the stream emits a `question` event whose
...
is not the goal's own `verified`.
...
(stamps `verified`, emits the `verified` event, exit 0) with no fresh agent pass. If the
