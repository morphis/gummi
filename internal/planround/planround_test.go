...
func (f *fakeStore) PlanRounds(_ context.Context, id domain.FeatureID) (int, error) {
	f.id = id
	f.load++
	return 3, f.loadErr
}

func (f *fakeStore) IncrementPlanRounds(_ context.Context, id domain.FeatureID) error {
	f.id = id
	f.bump++
	return f.writeErr
}

func (f *fakeStore) ClearPlanRounds(_ context.Context, id domain.FeatureID) error {
	f.id = id
	f.reset++
	return f.writeErr
}
...
func TestLoadCallsPlanRounds(t *testing.T) {
	f := &fakeStore{}
