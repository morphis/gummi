...
func goalBoard() *Shell {
	...
	g.Goal = &engine.GoalReport{
		...
		Cards:    []engine.GoalReportCard{{ID: "FD-011", State: "landed"}, {ID: "FD-012", State: "running"}},
	...
	c2.F.GoalID = g.F.ID
	m.rows = []featureRow{row(5, "dark mode", domain.StagePlan, "", false), g, c1, c2}
...
func TestBoardFoldsGoalCardsUnderTheirGoal(t *testing.T) {
	...
	order = m.displayOrder(m.sortMode)
	if len(order) != 4 || order[1] != 1 || order[2] != 2 || order[3] != 3 {
