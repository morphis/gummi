...
// follow-up "resume it?" question: an autopilot card whose envelope,
// before this raise, was already spent — the one shape of "parked for
// lack of budget" this dialog can tell from a card that's simply mid-run
// without reaching into live session state, which this dialog — built
// from a bare domain.Feature snapshot — has no access to.
//
// The mode is read through GateMode, never off the raw field: every card
// `bugs new` mints stores the empty string, and a bare comparison against
// domain.GateAttended reads every one of them as autopilot — which would
// hand an attended card the resume question, and a "yes" there restarts a
// run nobody handed over.
